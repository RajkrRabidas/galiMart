...
const startPaymentConsumer = async () => {
  try {
    ...
    if (!channel) {
      console.warn("⚠️ RabbitMQ channel not initialized; payment consumer skipped");
      return;
    }
    ...
    console.warn("⚠️ Failed to start payment consumer:", error.message);
  }
};
...
const startOrderConsumer = async () => {
  try {
    ...
    if (!channel) {
      console.warn("⚠️ RabbitMQ channel not initialized; order consumer skipped");
      return;
    }
    ...
    await channel.consume(process.env.RIDER_READY_QUEUE, async (msg) => {
      ...
      try {
        ...
        console.error("❌ OrderReadyForRider consumer error:", error.message);
        if (msg) channel.nack(msg);
      }
    });
    console.log("✅ Order consumer started successfully");
  } catch (error) {
    console.warn("⚠️ Failed to start order consumer:", error.message);
  }
};
