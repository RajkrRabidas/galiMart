import { Truck, Package, IndianRupee, CheckCircle } from "lucide-react";
...
const Dashboard = () => {
  ...
  const createProfile = async (event) => {
    ...
    const { name, phoneNumber, aadharNumber, aadharImage, drivingLicenseNumber, image } =
      profileForm;

    if (!name.trim() || !phoneNumber || !aadharNumber || !aadharImage || !drivingLicenseNumber || !image) {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          toast.success(
            `You are now ${!profile.isAvailable ? "available" : "unavailable"} for deliveries.`,
          );
        } catch (err) {
          toast.error(
    ...
    <div className="min-h-screen from-emerald-50 via-white to-slate-100 pb-24">
      <div className="max-w-7xl mx-auto p-6">
        ...
        <button
          ...
          disabled={toggling}
          className="mt-8 rounded-lg bg-emerald-600 px-5 py-3 font-semibold text-white disabled:opacity-60"
        >
          {toggling
            ? "Updating..."
            : profile.isAvailable
